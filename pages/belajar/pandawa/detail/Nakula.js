...
const audio_nakula = new Sound('audio_nakula.mp3', Sound.MAIN_BUNDLE, (error) => {
  if (error) {}     
});

const narasi_nakula = new Sound('narasi_nakula.mp3', Sound.MAIN_BUNDLE, (error) => {
  if (error) {}     
});

const DetailNakula: () => React$Node = () => {  
  
  setTimeout(play_narasi,2000);
  function play_narasi(){
    sound_pandawa.setVolume(.1)
    narasi_nakula.setVolume(.5).play(()=>{narasi_nakula.release()});
  }  
  ...
  function audio_play(){        
    narasi_nakula.stop();
    audio_nakula.play(()=>{audio_nakula.release()});
  }
    ...
    <View style={styles.container}>
      ...
      <View style={{flex: 1, flexDirection: 'column', backgroundColor:'transparent'}}>
              <View style={{ backgroundColor:'transparent', height:'20%', width:'100%', justifyContent:'center', left:120}}>                    
                    <Image 
                      source={require('../../../../../android/app/src/main/res/raw/n_nakula.png')} 
              ...
              <View style={{ backgroundColor:'transparent', height:'80%', flexDirection:'column', flex:1}}>
                  <View style={{backgroundColor:'transparent', height:'80%', width:'100%', flex:1, flexDirection:'row'}}>
                      <View style={{width:'40%', backgroundColor:'transparent'}}>
                        <Image source={require('../../../../../android/app/src/main/res/raw/nakula_1.png')} style={{width:90,height:'100%', right:-130}}></Image>
                      </View>
                      <View style={{width:'60%', alignItems:'center', backgroundColor:'transparent'}}>
                        ...
                        <Text style={styles.text}>Aku Nakula dan saudaraku sadewa adalah saudara kembar. Tentunya aku dan sadewa juga merupakan anak dari Ayah Prabu Pandu Dewanata dan Ibunda Dewi Madrin. Semasa kita kecil, kita tinggal di negara Astina bersama kakak-kakak kita. Menurut leluhurku kehadiran Kita melambangkan persahabatan yang kekal,  karena kemanapun kita pergi kita selalu bersama dan kerjasama. Kita selalu membela kebenaran, dan sayang kepada keluarga kita.</Text>
                        <Pressable
                              style={styles.tombol}
                              onPress={audio_play}
